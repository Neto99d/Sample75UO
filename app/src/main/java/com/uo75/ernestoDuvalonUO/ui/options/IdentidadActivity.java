...
import com.uo75.ernestoDuvalonUO.ui.options.Adapters.IdentidadAdapter;
...
public class IdentidadActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imageFull = findViewById(R.id.imageViewFullIdentidad);
        fondoImageFull = findViewById(R.id.cardViewFullImageIdentidad);
        mRecyclerView = findViewById(R.id.recycler_identidad);
        mRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        mRecyclerView.setItemAnimator(new DefaultItemAnimator());
        mRecyclerView.setHasFixedSize(true);
        progressBar = findViewById(R.id.progressBarIdentidad);
        ...
        call.enqueue(new Callback<AccesOdoo>() {
            ...
            public void onResponse(Call<AccesOdoo> call, Response<AccesOdoo> response) {
                ...
                if (response.isSuccessful()) {//si la peticion se completo con exito
                    ...
                    ArrayList<Identidad> identidads = new ArrayList<>();
                    try {
                        OkHttpUtil.init(true);
                    } catch (Exception e) {
                        e.printStackTrace();
                    ...
                    Call<Data> callS = service.getIdentidad(params);
                            ...
                            if (response.isSuccessful()) {//si la peticion se completo con exito
                                ...
                                try {
                                    ...
                                    for (int i = 0; i < data.getData().size(); i++) {
                                        ...
                                        identidad.setImagen(StringToBitMap(data.getData().get(i).get("imagen").getAsString().substring(data.getData().get(i).get("imagen").getAsString().indexOf("'") + 1)));
                                        identidads.add(identidad);
                                    }
                                    if (identidads.isEmpty()) {
                                    ...
                                    nAdapter = new IdentidadAdapter(identidads, mContext, imageFull, fondoImageFull);
